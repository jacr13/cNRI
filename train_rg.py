...
def run(mode, data_loader, encoder, decoder, optimizer, rel_rec,
        rel_send, log_prior, skeleton, args):
    ...
    for batch_idx, (data, clinical) in enumerate(data_loader):
        ...
        edges = np.repeat(skeleton[np.newaxis, :, :], data.shape[0], axis=0)
        ...
        loss_kl = Variable(torch.Tensor([0])).to(args.device)
...
def generations(data_loader, decoder, rel_rec, rel_send, skeleton,
...
def main(args):
    ...
    # optimizer
    optimizer = optim.Adam(list(decoder.parameters()),
                           lr=args.lr)
    ...
    for epoch in range(1, args.epochs+1):
        _ = train(epoch, train_loader, None, decoder, optimizer, scheduler,
                  rel_rec, rel_send, log_prior, skeleton, args, logger)

        val_loss, _ = test('val', epoch, val_loader, None, decoder, rel_rec,
                           rel_send, log_prior, skeleton, args, logger)

        _, _ = test('test', epoch, test_loader, None, decoder, rel_rec,
                    rel_send, log_prior, skeleton, args, logger)
    ...
    generations(test_loader, decoder, rel_rec, rel_send, skeleton,
                first_frame_params, args, logger)
